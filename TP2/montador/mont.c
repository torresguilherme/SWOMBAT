...
int main(int argc, char** argv)
{
	...
	FILE *objeto = fopen(nome_do_arquivo, "w+");
	FILE *memoria_de_dados = tmpfile();
	FILE *tabela_de_simbolos = fopen("tabela.txt", "w+");
	char aux[300];
  ...
  //PASSAGEM2
	while(fscanf(entrada, "%[^\n]s", aux) != EOF)
	{
		...
		if(instrucao != -1)
		{
			...
			//offset do endereco
			fprintf(objeto, "%04x", endereco++);

			//tipo do dado
			fprintf(objeto, "00");

			//dado da instrucao - primeira parte
			fprintf(objeto, "%02x", (instrucao >> 8) & (255));
			...
			fprintf(objeto, "%02x", instrucao & (255));
			...
			complemento--;
			complemento = (~complemento);
			fprintf(objeto, "%02x", complemento & (255));
	...
	{
		fprintf(objeto, ":01%04x0000", endereco);
		complemento = endereco;
		complemento = (~complemento);
		fprintf(objeto, "%02x\n", complemento & (255));
	}
